...
from systems.docforge.agent_loop import (
    enter_docforge, exit_docforge, run_phase, get_available_phases
)


class TestDocForgeFiles:
...
class TestDocForgeAgentLoop:
    """Test the agent_loop functions."""

    def test_enter_returns_string(self):
        result = enter_docforge()
        assert isinstance(result, str), "enter_docforge should return str"

    def test_exit_returns_string(self):
        result = exit_docforge()
        assert isinstance(result, str), "exit_docforge should return str"
