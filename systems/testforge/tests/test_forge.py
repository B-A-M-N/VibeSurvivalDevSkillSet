...
from systems.testforge.agent_loop import (
    enter_testforge, exit_testforge, run_phase, get_available_phases
)


class TestTestForgeFiles:
...
class TestTestForgeAgentLoop:
    """Test the agent_loop functions."""

    def test_enter_returns_string(self):
        result = enter_testforge()
        assert isinstance(result, str), "enter_testforge should return str"

    def test_exit_returns_string(self):
        result = exit_testforge()
        assert isinstance(result, str), "exit_testforge should return str"
